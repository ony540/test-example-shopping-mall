...
describe('로그인이 된 경우', () => {
  ...
  it('장바구니(담긴 상품 수와 버튼)와 로그아웃 버튼(사용자 이름: "boo")이 노출된다.', async () => {
    await render(<NavigationBar />);
    expect(screen.getByTestId('cart-icon')).toBeInTheDocument();
    expect(screen.getByText('2')).toBeInTheDocument();
    expect(
      await screen.findByRole('button', { name: 'boo' }),
    ).toBeInTheDocument();
  });
  ...
  describe('로그아웃 버튼(사용자 이름: "Maria")을 클릭하는 경우', () => {
    ...
    it('모달의 확인 버튼을 누르면, 로그아웃이 되며, 모달이 사라진다.', async () => {
      ...
      //not - 사용자 정보가 없고 모달도 사라진다
      expect(
        screen.queryByRole('button', { name: 'boo' }),
