...
it('장바구니에 포함된 아이템들의 이름, 수량, 합계가 제대로 노출된다', async () => {
  ...
  expect(
    within(firstItem).getByText('Handmade Cotton Fish'), // firstItem 안에서 '~~'text를 가진 요소를 조회
  ).toBeInTheDocument();
...
it('특정 아이템의 수량이 1000개로 변경될 경우 "최대 999개 까지 가능합니다!"라고 경고 문구가 노출된다', async () => {
  // stubGlobal - 돔 내 기본 함수(1번째 인자)를 2번째 인자로 받은 함수로 모킹
...
it('특정 아이템의 삭제 버튼을 클릭할 경우 해당 아이템이 사라진다', async () => {
  ...
  // queryBy~~~: 요소 존재 유뮤 판단. 요소가 존재하지 않아도 에러X
  expect(screen.queryByText('$4,045.00')).toBeInTheDocument();
});
